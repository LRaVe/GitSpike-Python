...
def simulated_annealing(spikes, Tmax, params_sa, verbose=False, **kwargs):
    ...
    while True:
        ...
        row_state = np.zeros(N, dtype=int)
        row_state[current_pop] = 1
        matrix_grid_list.append(row_state)
        history_perf_list.append(P_current)
    ...
    result = {
        ...
        'matrixGrid': np.array(matrix_grid_list),      
        'historyPerf': np.array(history_perf_list),
